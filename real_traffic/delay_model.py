...
class RouteNet_Fermi(nn.Module):
    def __init__(self):
        ...
        # Readout network for occupancy prediction
    ...
    def forward(self, inputs):
        ...
        # Calculate load and packet size
        ...
        # Process states for delay calculation
        path_states_tensor = torch.stack(path_states_sequence, dim=1)
        capacity_gather = torch.gather(capacity, 1, link_to_path)
        
        # Calculate occupancy and delays
        occupancy_gather = self.readout_path(path_states_tensor)
        # Handle variable length sequences
        mask = torch.arange(occupancy_gather.size(1)).expand(length.size(0), -1) < length.unsqueeze(1)
        occupancy_gather = occupancy_gather * mask.unsqueeze(-1).float()
        
        # Calculate queueing delay
        queue_delay = torch.sum(occupancy_gather / capacity_gather.unsqueeze(-1), dim=1)
